...
SEARCH_BY = ["Name", "Zip", "Type", "Visits", "Avg. Rating"]
...
class OwnerWelcomeWindow(Frame):
    def __init__(self, master, db_cursor):
        Frame.__init__(self, master)

        self.db_cursor = db_cursor

        self.nameofowner = self.master.master.logged_in_user;

        self.welcome_label = Label(self,
                           text="Welcome {}".format(self.nameofowner),
                           font="Times 36")
        ...
        self.add_prop_button = Button(self.button_container,
                                         text="Add Property",
                                         padx=10,
                                         command=self.add_prop_button_clicked_handler)
        self.add_prop_button.pack(side=LEFT, padx=(0, 50))
    ...
    def sort_button_click_handler(self):
        ...
        if sort_attr == SEARCH_BY[0] or sort_attr == SEARCH_BY[1]:
            self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) AS AvgRating, COUNT(Visit) AS Visits
            ...
            self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) AS AvgRating
                                   ...
                                   WHERE Owner = {}
                                   GROUP BY Name
                                   ORDER BY AvgRating""".format(PROP_ATTRS, self.nameofowner))
    ...
    def search_button_click_handler(self):
        ...
        if search_attr == SEARCH_BY[0]:
            ...
            self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) COUNT(Visit) AS Visits
                                   FROM Property LEFT OUTER JOIN Visit
                                   ON ID=PropertyID
                                   WHERE OWNER = {} AND {}=\"{}\"
                                   GROUP BY Zip""".format(PROP_ATTRS, search_attr, search_val));
        elif search_attr == SEARCH_BY[2]:
            self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) AS AvgRating, COUNT(Visits) AS Visits
                                   FROM Property LEFT OUTER JOIN Visit
                                   ON ID=PropertyID
                                   WHERE OWNER = {} AND {}=\"{}\"
            ...
            self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) AS AvgRating, COUNT(Visits) AS Visits
                                   ...
                                   ON ID=PropertyID
                                   WHERE OWNER = {} AND {}=\"{}\"
                                   GROUP BY Visits""".format(PROP_ATTRS, self.nameofowner, search_attr, search_val))
        else:
            try:
                ...
                self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1) AS AvgRating, COUNT(Visits) AS Visits
                                       ...
                                       WHERE OWNER = {}
                                       GROUP BY Name
                                       HAVING AvgRating>={} AND AvgRating<={}""".format(PROP_ATTRS, self.nameofowner,
    ...
    def manage_prop_button_clicked_handler(self):
            ...
            self.master.master.windows["OwnerManagePopertyWindow"].fill_in_data_from_prop(prop)
            self.master.master.windows["OwnerManagePopertyWindow"].get_approved_animals_and_crops_from_db()
            self.master.master.windows["OwnerManagePopertyWindow"].set_previous_window("OwnerWelcomeWindow")
            self.master.master.show_window("OwnerManagePopertyWindow")
              except:
            messagebox.showinfo("Alert", "Please Select a Property to Manage From the Table.")

    def add_prop_button_clicked_handler() 
        self.master.master.show_window("ShowNewPropertyWindow")
    ...
    def init_populate_table(self):
        self.populate_table("""SELECT {}, ROUND(AVG(Rating), 1),  COUNT(Visits) AS Visits
                               FROM Property LEFT OUTER JOIN Visit
                               WHERE OWNER = {}
