...
CROP_TYPE = ["FLOWER", "NUT", "FRUIT", "VEGETABLE", "ANIMAL"]

class OwnerManagePropertyWindow(Frame):
...
# Requesting the Animal/Crop you want
        self.request_animals_or_crops_container_request_crops=Frame(self.add_animals_or_crops_container)
        self.request_animals_or_crops_container_request_crops.pack(side=RIGHT)

        

        self.request_label = Label(self.request_animals_or_crops_container_request_crops,
                           text="Request Animal/Crop Approval:",
                           font="Times 12")
        self.request_label.pack(side=LEFT, pady=(50,0))
        
        self.reqcrop_entry = Entry(self.request_animals_or_crops_container_request_crops,
                                font="Times 12",
                                width=12)
        self.reqcrop_entry.pack(side=LEFT, pady=(50,0))

        self.reqcrop_var = StringVar(self.basic_info_container_labels_part_2_right_side)
        self.reqcrop_var.set(CROP_TYPE[1])

        self.reqcrop_drop_down = OptionMenu(self.request_animals_or_crops_container_request_crops, self.reqcrop_var, *CROP_TYPE)
        self.reqcrop_drop_down.pack(side=LEFT, pady=(50,0))

        self.request_animals_or_crops_container_request_crops_select_side=Frame(self.request_animals_or_crops_container_request_crops)
        self.request_animals_or_crops_container_request_crops_select_side.pack(side=RIGHT)

        self.req_cr_button = Button(self.request_animals_or_crops_container_request_crops_select_side,
                                   text="Request Crop",
                                   padx=10,
                                   height=2,width=12,
    ...
    def req_crop_button_clicked_handler(self):
        if messagebox.askyesno("Alert", "Are You Sure You Want to Request This Animal or Crop?"):
            checkquery = """SELECT CASE WHEN EXISTS (
                            ...
                            WHERE Name = \"{}\" AND Type = \"{}\")
                            THEN 'true'
                            ELSE 'false' END""".format(self.reqcrop_entry.get(), self.reqcrop_var.get())
